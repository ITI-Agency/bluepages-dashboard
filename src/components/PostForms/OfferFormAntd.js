...
function OfferFormAntd({ onSubmit, offerDetials }) {
  const { useForm, Item } = Form;
  const [form] = useForm();
  useEffect(() => {
    if (offerDetials) {
      form.setFieldsValue({
        name_en: offerDetials.name_en,
        name_ar: offerDetials.name_ar,
      });
    }
  }, [offerDetials]);
