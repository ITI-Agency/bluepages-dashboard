...
const getAllVideos = async (filters = []) => {
	...
	const data = await fetch(`${process.env.REACT_APP_API_BASE_URL}offerVideos${queries}`, {
		headers: {
...
const getVideoDetails = async (VideoId) => {
	const AUTH_JWT = sessionStorage.getItem("AUTH_JWT");
	const data = await fetch(`${process.env.REACT_APP_API_BASE_URL}offerVideos/${VideoId}`, {
		headers: {
			'Authorization': `Bearer ${AUTH_JWT}`
		}
	});
